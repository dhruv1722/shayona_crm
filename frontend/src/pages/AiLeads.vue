  ...
  >
    <template #title="{ titleField, itemName }">
      <div class="flex items-center gap-2">
        ...
        <div v-else-if="titleField === 'mobile'">
...
function actions(itemName) {
  let mobile = getRow(itemName, 'mobile')?.label || ''
  let actions = [
    {
      icon: h(PhoneIcon, { class: 'h-4 w-4' }),
      label: __('Make a call'),
      onClick: () => makeSmartFlowCallFromList(itemName, mobile),
      condition: () => !!mobile,
    },
    {
...
async function makeSmartFlowCallFromList(aiLead, mobile) {
  if (!mobile) {
